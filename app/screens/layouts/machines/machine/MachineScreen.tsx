...
import { Customtable, CustomDropdown, LoadingSpinner, Inputs, Searchbar, AccessibleView } from "@/components";
...
const MachineGroupScreen = () => {
    ...
    const dropmachine = useMemo(() => {
        return Array.isArray(machineGroup)
            ? machineGroup.filter(
                (v) => v.IsActive || v.MachineID === initialValues.machineGroupId
            )
            : [];
