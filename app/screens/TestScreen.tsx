...
const DragAndDropForm = () => {
  ...
  const handleDrop = () => {
    if (draggedItem && !isDropped.value) {
      ...
      // Mark as dropped but don't clear dragged item, so it can be dragged again
    }
  };
    ...
    return (
      <PanGestureHandler
        ...
        onEnded={(e: PanGestureHandlerGestureEvent) => {
          // Check if the item is being dropped on the right side
          if (e.nativeEvent.absoluteX > width / 2) {
            handleDrop(); // Call handleDrop if the item is dropped on the right
          }
